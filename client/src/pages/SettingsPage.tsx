    ...
    <div className="page">
      ...
      <div className="settings-grid">
              ...
              {(settings.subAgentMode === "manual" || settings.subAgentMode === "realtime" || settings.subAgentMode === "auto_swarm" || settings.subAgentMode === "auto_create") ? (
                <>
                  ...
                  <div className="form-actions" style={{ gap: 8, flexWrap: "wrap" }}>
                    ...
                    {uploadError && (
                      <p style={{ width: "100%", margin: 0, color: "var(--error)", fontSize: 12 }}>{uploadError}</p>
                    )}
        ...
        <section className="card">
          ...
          <p className="hint" style={{ marginBottom: 12 }}>
            Allow this instance to accept peer-to-peer connections from other CeTeau AI machines. When disabled, remote token authentication is rejected and the remote menu is hidden.
          </p>
        </section>
        ...
        <section className="card">
          ...
          <p className="hint" style={{ marginBottom: 12 }}>
            Delegate tasks to CeTeau AI running on other machines. Both machines run the same codebase — either can be orchestrator or worker.
          </p>
          ...
          {(settings.remoteInstances || []).map((ri: any) => (
            <div key={ri.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 0", borderBottom: "1px solid var(--border)", fontSize: 13 }}>
              ...
              {remoteTestResults[ri.id] && (
                <span style={{ fontSize: 11, color: remoteTestResults[ri.id]!.ok ? "var(--success)" : "var(--error)" }}>
        ...
        <section className="card">
          ...
          <textarea
            ...
            style={{
              ...
              padding: 12, borderRadius: 6, border: `1px solid ${mcpJsonError ? "var(--error)" : "var(--border)"}`,
    ...
    "web-search-prime": {
      ...
      "headers": {
        "Authorization": "Bearer your_api_key"
          ...
          />
          {mcpJsonError && (
            <p style={{ color: "var(--error)", fontSize: 12, margin: "4px 0 0" }}>{mcpJsonError}</p>
          )}
          {mcpJsonDirty && (
              ...
              {mcpStatuses.map((s) => (
                <div key={s.name} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: "1px solid var(--border)", fontSize: 13 }}>
                  <span style={{
                    ...
                    background: s.connected ? "var(--success)" : "var(--error)",
                    display: "inline-block", flexShrink: 0,
                  }} />
                  <strong style={{ flex: 1 }}>{s.name}</strong>
                  {s.connected ? (
                    <span style={{ fontSize: 11, color: "var(--success)" }}>{s.toolCount} tools</span>
                  ) : (
                    <span style={{ fontSize: 11, color: "var(--error)" }}>disconnected</span>
                  )}
                </div>
              ))}
            </div>
